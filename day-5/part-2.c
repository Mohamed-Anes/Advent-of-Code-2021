...
int main()
{
    ...
    while(fscanf(new_file, "%s %s %s %s", x1, y1, x2, y2) == 4)
    {
        ...
        }else //case of diagonal
        {
            s = (p2.x > p1.x)? p1.x : p2.x;  //s_x
            b = (s == p1.x)? p2.x : p1.x;  //b_x
            int s_y = (s == p1.x)? p1.y : p2.y;
            int b_y = (b == p1.x)? p1.y : p2.y;
            int direction = (b_y - s_y > 0)? 1 : -1;

            for(int i = 0; i <= (b - s); ++i)
            {
                (map[s + i][s_y + direction*i])++;
            }
