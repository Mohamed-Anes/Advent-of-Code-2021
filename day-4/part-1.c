...
int check_win(Grid *grids, int nb_boards);
...
int main()
{
    ...
    while(str_number != NULL)
    {
        ...
        if((winning_board = check_win(grids, nb_boards)) != -1)
        {
            break;
        }
    ...
    printf("last number : %d  winning board : %d\n\n", number, winning_board);
    ...
    {
        if(!grids[winning_board].numbers[i].is_marked)
        {
            score += grids[winning_board].numbers[i].value;
        }
    }
    score *= number;
